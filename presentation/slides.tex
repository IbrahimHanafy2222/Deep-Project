...
\begin{itemize}
  \item Each pair \textrightarrow{} two samples: corrupted = ungrammatical (0),
        clean = grammatical (1).
  \item Features: TF--IDF, unigrams + bigrams, 50k vocab (fit on train only).
  \item Classifiers: Logistic Regression \& Linear SVM.
\end{itemize}
\vspace{1em}
\begin{block}{Result}
$\sim$\textbf{62\,\%} detection accuracy --- barely above 50\,\% chance.\\
Surface word statistics cannot solve GEC. \textrightarrow{} need seq2seq.
\end{block}
\end{frame}
...
\centering
\includegraphics[width=0.72\textwidth]{../report/figures/length_bucketed_compare.png}
\vspace{0.4em}

\small All models do best on short sentences; the bottleneck LSTM degrades
fastest as sentences get longer.
\end{frame}
